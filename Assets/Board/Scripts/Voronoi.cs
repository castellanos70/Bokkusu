...
public class Voronoi
{
    ...
    public void next()
    {
        for (int i = 0; i < POINTCOUNT; i++)
        {
            ...
            {
                pointListX[i] = 0;
                speedX[i] = Random.Range(1, 10);
            }
            if (pointListY[i] < 0)
            {
                pointListY[i] = 0;
                speedY[i] = Random.Range(1, 10);
            }
            if (pointListX[i] > pixelSize - 1)
            {
                pointListX[i] = pixelSize - 1;
                speedX[i] = Random.Range(-10, -1);
            }
            if (pointListY[i] > pixelSize - 1)
            {
                pointListY[i] = pixelSize - 1;
                speedY[i] = Random.Range(-10, -1);
            }
        }
    ...
    private void randomizePoints()
    {
        for (int i = 0; i < POINTCOUNT; i++)
        {
            ...
            speedX[i] = Random.Range(-10, 10);
            speedY[i] = Random.Range(-10, 10);
        }
    }
}
